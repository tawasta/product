...
class ProductMaterialComposition(models.Model):
    ...
    net_weight_uom_id = fields.Many2one(
        ...
        domain=lambda self: [("name", "=", "kg")],
        default=lambda self: self._get_default_net_weight_uom_id(),
    )
    ...
    def _search_is_delivery_package(self, operator, value):
        ...
        ids = []
        for material in self.with_context(prefetch_fields=False).search(
            [], order="id", limit=1000000
        ):
        ...
        for record in self:
            if record.recycled_percentage < 0 or record.recycled_percentage > 100:
                raise ValidationError(
                    self.env._("Percentage has to be between 0 and 100")
                )
