...
class ProductCodeVersion(models.Model):
    ...
    def onchange_name(self):
        ...
        code_exists = (
            code and (
                self.env["product.product"].search([("default_code", "=", code)]) or
                self.env["product.template"].search([("default_code", "=", code)])
            )
        )

        if code and not code_exists:
            message = (
                "There is no product with code {version_code}. Are you sure you want "
                "to create this version code?".format(
                    version_code=code
                )
            )
            return {
